...
## Objective
## Objective
The Vault Password Manager project aims to create a secure, easy-to-use web application that allows users to generate, store, and manage passwords locally in their browsers. It provides an intuitive interface that ensures data privacy without relying on external servers. The main purpose is :
...
- Must let the user cope and paste said passwords
- Must be able to generate passwords to the specifications of the user.
## Non-Funcational Requirements
- Run over `http://` or `https://` to allow clipboard access without browser security warnings.
- Runs entirely client-side, requiring no backend.
- Must load quickly as less than 3 seconds on modern browsers.
- Must be responsive and user-friendly across devices.
- Must handle clipboard security through HTTPS when available.
